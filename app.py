...
def customer_login():
    if request.method == 'POST':
        ...
        if user and user.check_password(password) and user.role == 'buyer' and user.status == 'active':
            login_user(user)
            add_audit('Customer login', f'User {user.email} logged in')
...
def repair_request():
    ...
    if request.method == 'POST':
        ...
        repair = Repair(user_id=current_user.id, device=device, issue=issue, status='pending')
...
def vendor_dashboard():
    ...
    orders = Order.query.all()  # in real app, filter by vendor
    earnings = sum([p.price for p in products]) * get_settings().vendor_commission
    return render_template('vendor_dashboard.html', products=products, orders=orders, earnings=earnings)
...
def vendor_workers():
    if current_user.role != 'vendor':
        flash('Access denied', 'danger')
        return redirect(url_for('index'))
    workers = User.query.filter_by(status='active').filter(User.role.in_(['technician', 'agent', 'installer'])).all()
...
def superadmin_restore():
    ...
    if file:
        data = json.load(file)
        # simplistic restore – only non‑destructive demo
        flash('Restore feature – would overwrite data. Implement carefully.', 'warning')
    else:
...
# To avoid separate files, we embed all templates in the Python script.
# Flask will still find them if placed in 'templates' folder, but we define them as strings for self‑contained deployment.
            ...
            {% if cart_items %}
                <table class="table table-dark">
                    ...
                    {% for item in cart_items %}
                        ...
                        <tr>
                            <td>{{ item.product.name }}</td><td>UGX {{ item.product.price|int }}</td>
...
# Override template loader to serve embedded templates
@app.route('/<template_name>')
def serve_template(template_name):
    return "Not found", 404

# Monkey‑patch Flask to use our embedded templates
import os
    ...
    with app.app_context():
        ...
        if not Product.query.first():
            demo = Product(name='Samsung Galaxy A54', price=1250000, market_price=1650000, category='Smartphones', stock=25,
                           image_url='https://images.unsplash.com/photo-1610945265064-0e34e5519bbf?w=400', description='6.4" display')
            db.session.add(demo)
            db.session.commit()
    app.run(debug=True)
    if __name__ == '__main__':
